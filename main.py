...
def main():
    ...
    workflow.add_node("adjust_fitness", lambda x: fitness_agent.start(x) if x else fitness_agent.current_workout_plan)
    workflow.add_node("nutrition", lambda _: nutrition_agent.start())
    workflow.add_node("collect_nutrition_feedback", lambda _: collect_feedback("nutrition"))
    workflow.add_node("adjust_nutrition",
    ...
    workflow.add_node("collect_mental_health_feedback", lambda _: collect_feedback("mental health"))
    workflow.add_node("adjust_mental_health",
                      lambda x: mental_health_agent.start(x) if x else mental_health_agent.wellness_tips)
    workflow.add_node("progress_report", lambda _: progress_agent.track_progress(
